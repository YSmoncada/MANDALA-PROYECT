...
function HeaderPedidosDisco({ user, onLogout, codigoConfirmado }) {
  ...
  const activeClasses =
    "bg-zinc-900 dark:bg-white text-white dark:text-black border border-white/10 dark:border-white shadow-xl";
  ...
  return (
    ...
    bg-gradient-to-r from-[#F8F9FC] to-[#EDEBFF]
    dark:bg-black/95
    backdrop-blur-xl
        ...
        <div className="w-full px-8">
          <div className="flex items-center justify-between h-20">
            ...
            <div className="flex items-center gap-10">
              {/* Desktop Navigation */}
              <nav className="hidden md:flex items-center gap-4 bg-[#2B1055]/60 dark:bg-zinc-900/50 p-1.5 rounded-2xl border border-white/5">
                {navLinks.map((link) => {
                  const isEnabled =
                  ...
                  return isEnabled ? (
                    <Link
                      key={link.href}
                      to={link.href}
                      className={`flex items-center gap-3 px-6 py-2.5 rounded-xl transition-all duration-500 font-black text-[10px] uppercase tracking-[0.2em] shadow-2xl ${
                        isActive
                ...
                {user && (
                  <div className="flex items-center gap-6 border-l border-white/10 dark:border-white/5 pl-6">
                    <div className="flex flex-col items-end">
                      <span className="text-white dark:text-zinc-200 font-black text-xs uppercase tracking-widest italic">
                        {user}
                      </span>
                      <div className="flex items-center gap-2 mt-0.5">
                        <div className="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-pulse"></div>
                        <span className="text-[9px] text-emerald-400 font-black tracking-[0.2em] uppercase">
                          Online
                        </span>
                      </div>
                    </div>
              ...
              {navLinks.map((link) => {
                ...
                return isEnabled ? (
                  <Link
                    ...
                    className={`flex items-center gap-5 px-6 py-5 rounded-2xl font-black uppercase tracking-[0.2em] text-xs transition-all ${
                      isActive

        ...
        <div className="bg-[#1A103C]/80 backdrop-blur-xl border border-[#A944FF]/30 rounded-2xl p-8 relative overflow-hidden shadow-[0_0_50px_rgba(169,68,255,0.1)]">
            ...
            {sortedVentas.length > 0 ? (
                <div className="relative h-96 w-full">
                    <div className="absolute inset-0 flex flex-col justify-between text-[10px] text-gray-600 pointer-events-none z-0 pb-10 pr-4">
                        {[100, 75, 50, 25, 0].map((percent) => (
                            <div key={percent} className="w-full border-b border-gray-700/30 flex items-center h-px">
                        ...
                        {sortedVentas.map((dia, idx) => {
                            ...
                            return (
                                <div key={idx} className="flex-1 flex flex-col items-center gap-4 group h-full justify-end">
                                    <div
                                        className="w-full max-w-[60px] bg-gradient-to-t from-[#441E73] to-[#A944FF] rounded-t-xl relative transition-all duration-700 hover:shadow-[0_0_25px_rgba(169,68,255,0.5)] hover:brightness-125 group-hover:scale-y-105 origin-bottom cursor-pointer shadow-lg"
                                        style={{ height: `${heightPerc}%`, minHeight: '6px' }}
                                    >
                                        <div className="absolute top-0 left-0 right-0 h-1 bg-white/40 rounded-t-xl"></div>
                                        <div className="absolute -top-14 left-1/2 -translate-x-1/2 bg-[#0E0D23] border border-[#A944FF] text-[10px] font-black px-4 py-2 rounded-xl opacity-0 group-hover:opacity-100 transition-all transform translate-y-4 group-hover:translate-y-0 whitespace-nowrap shadow-2xl z-20 pointer-events-none text-white italic">
                                    ...
                                    <div className="text-center group-hover:translate-y-1 transition-transform">
                                        <span className="text-[10px] font-black text-gray-300 block mb-0.5 uppercase tracking-tighter">
                ...
                <div className="h-96 flex flex-col items-center justify-center text-gray-500 border border-dashed border-gray-700/50 rounded-[40px] bg-black/40">
                    <div className="p-6 bg-white/5 rounded-full mb-4">
                       <PieChart size={48} className="text-gray-700" />
                    </div>
